...
/**
 * Vietnamese developer publishing platform. Only the "newest" RSS feed is public
 * (`/search` and `*.json` are disallowed by robots.txt), so each run picks up
 * recently published articles and the store accumulates them over time.
...
export const vibloCrawler: CommunityCrawler = {
  ...
  async run({ config, aliasIndex, now, log }: CrawlContext): Promise<CommunityPost[]> {
    ...
    const xml = await fetchText(`${ORIGIN}/rss`, {
